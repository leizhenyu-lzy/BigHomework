...
void myConvertFASTMatToVector(const Mat &harrisThresholdMat, vector<pair<int, int>> &cornerPoints)
{
    const int Rows = harrisThresholdMat.rows;
    const int Cols = harrisThresholdMat.rows;
    ...
    // cout << "[myConvertFASTMatToVector] :: Vector's Size [" << cornerPoints.size() << "]." << endl;
    // auto pb = cornerPoints.begin();
    // auto pe = cornerPoints.end();
    // int counter = 0;
    // while(pb!=pe)
    // {
    //     cout<<counter<<" "<<(*pb).first<<" "<<(*pb).second<<endl;
    //     counter++;
    //     pb++;
    // }
}